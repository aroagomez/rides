...
public class FindRidesGUI extends JFrame {
	...
	private String[] columnNamesRides = new String[] {
			...
			ResourceBundle.getBundle("Etiquetas").getString("FindRidesGUI.CalificacionMedia"),
			...
			ResourceBundle.getBundle("Etiquetas").getString("FindRidesGUI.Ride")
	};
				...
				else if (propertychangeevent.getPropertyName().equals("calendar"))
				{
					...
					try {
						...
						for (domain.Ride ride:rides){
							...
							row.add(ride.getDriver().getCalificacionMedia());
		...
		btnErreserbatu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(pos == -1) {
					...
					Ride ride = (Ride) tableModelRides.getValueAt(pos, 4);
