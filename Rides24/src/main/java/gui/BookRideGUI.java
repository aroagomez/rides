...
public class BookRideGUI extends JFrame {
	...
	public BookRideGUI(Ride ride, User user) {
		...
		lblDescuento = new JLabel(""); //$NON-NLS-1$ //$NON-NLS-2$
		lblDescuento.setBounds(113, 108, 209, 13);
		panelReserva.add(lblDescuento);
		
		btnRes = new JButton();
		btnRes.setText(ResourceBundle.getBundle("Etiquetas").getString("BookRideGUI.Erreserbatu"));
		btnRes.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int numAsientos = (int) listaAsientos.getSelectedItem();
				Traveler traveler = (Traveler) user;
				if ((traveler.getNumRides()%10 == 0) && (traveler.getNumRides() != 0)){
					
					lblDescuento.setText(ResourceBundle.getBundle("Etiquetas").getString("BookRideGUI.Descuento"));
					
					if((numAsientos*ride.getPrice()/2)<=traveler.getMoney()) {
						Reserva res = new Reserva(ride.getDriver(), traveler, ride, numAsientos);
						res.setAceptada(false); //esta pendiente de confirmar
						
						BLFacade facade = MainGUI.getBusinessLogic(); 
						Boolean guardado = facade.guardarReserva(res);
						
						
						//JFrame a = new PagoGUI(); a.setVisible(true);
						 
						
						if(guardado) {
							lblError.setText(ResourceBundle.getBundle("Etiquetas").getString("BookRideGUI.pendiente"));
						}else {
							lblError.setText(ResourceBundle.getBundle("Etiquetas").getString("BookRideGUI.EzDaGorde"));
						}
					}
					else {
						System.out.println("No tienes dinero suficiente para esta reserva");
						lblError.setText(ResourceBundle.getBundle("Etiquetas").getString("BookRideGUI.NoDineroSufi"));
					}
				}else {
					if(numAsientos*ride.getPrice()<=traveler.getMoney()) {
						Reserva res = new Reserva(ride.getDriver(), traveler, ride, numAsientos);
						res.setAceptada(false); //esta pendiente de confirmar
		...
		btnRes.setBounds(75, 202, 126, 51);
		panelReserva.add(btnRes);
