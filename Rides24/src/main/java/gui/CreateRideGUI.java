...
public class CreateRideGUI extends JFrame {
	...
	private JLabel lblPrice =  new JLabel(ResourceBundle.getBundle("Etiquetas").getString("CreateRideGUI.Price"));
	...
	public CreateRideGUI(Driver driver) {
		...
		jLabelSeats.setBounds(new Rectangle(6, 119, 84, 20));
		...
		jTextFieldPrice.setBounds(new Rectangle(139, 119, 60, 20));
		jTextFieldPrice.setBounds(139, 150, 60, 20);
		getContentPane().add(jTextFieldPrice);
		lblPrice.setBounds(6, 153, 61, 17);
		
		getContentPane().add(lblPrice);
		
		SeguroCheckBox = new JCheckBox(ResourceBundle.getBundle("Etiquetas").getString("CreateRideGUI.AsegurarViaje")); //$NON-NLS-1$ //$NON-NLS-2$
		SeguroCheckBox.setBounds(28, 197, 241, 21);
		getContentPane().add(SeguroCheckBox);
		
		JLabel lblError = new JLabel(""); //$NON-NLS-1$ //$NON-NLS-2$
		lblError.setBounds(28, 224, 255, 20);
		getContentPane().add(lblError);
		
		if (SeguroCheckBox.isSelected()) {
			if (driver.getMoney() < 2) {
				lblError.setText(ResourceBundle.getBundle("Etiquetas").getString("CreateRideGUI.NoDineroSufi"));
...
//			
	...
	private void jButtonCreate_actionPerformed(ActionEvent e) {
		...
		if (error!=null) 
			...
			try {
				...
				Ride r=facade.createRide(fieldOrigin.getText(), fieldDestination.getText(), UtilDate.trim(jCalendar.getDate()), inputSeats, driver.getEmail(), Float.parseFloat(jTextFieldPrice.getText()), SeguroCheckBox.isSelected());
	...
	private String field_Errors() {
		...
		try {
			if ((fieldOrigin.getText().length()==0) || (fieldDestination.getText().length()==0) || (jTextFieldSeats.getText().length()==0))
